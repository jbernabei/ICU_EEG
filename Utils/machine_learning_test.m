function [results_struct] = machine_learning_test(model_struct,sz_list, partition_sz, nsz_list, partition_nsz, iic_list, partition_iic)

    % Set up folds
    for k = 1:length(model_struct)
        pt_test_list = [sz_list(partition_sz==k); nsz_list(partition_nsz==k)];% iic_list(partition_iic==k)];
         
        for i = 1:length(pt_test_list)
            
            % Extract patient number
            pt_no = pt_test_list(i);
            load(sprintf('feats_3_sec_%d.mat',pt_no));
